...
As of 2026-06-03 the ontology includes the **deploy convention**: repos that ship to a host carry `DEPLOY.md` (runbook), `scripts/deploy.sh` (the steps), and `deploy.json` (declarative post-deploy invariants that gate the deploy). The checks are run by the central `~/dev/ops` kit via `devops check` (which replaced the earlier per-repo `scripts/postdeploy-check.sh`); `devops lad-config` generates a local-only `## Deploy Configuration` block in `CLAUDE.md` that `/land-and-deploy` reads. The architect's gap report flags deployable repos missing any of the three tracked artifacts. Reference implementation: `sms-hero`.

As of 2026-06-09 the per-repo agent-file set is **four files**, not three: `CLAUDE.md` / `LOG.md` / `INDEX.md` / `PLAYBOOK.md`. `PLAYBOOK.md` is the canonical record of the repeatable **plays** a repo runs (a play is born as prose and graduates into a skill once it proves out, keeping the judgment and pointing at the skill for the mechanics). Unlike LOG.md / INDEX.md it is **git-tracked**, because it is a durable, reviewable process narrative and build backlog that must survive a fresh clone; CLAUDE.md carries a one-line pointer to it and it loads on demand, not every session. The architect's gap report now checks for all four files and harvests the prose plays across every PLAYBOOK.md into a cross-repo "plays to systematize" candidate-skills list (a derived view recomputed each run; the PLAYBOOK.md files are the source of truth, so there is no separate tracked backlog file). Reference implementation: `~/dev/businesses/dxangels/PLAYBOOK.md`.
